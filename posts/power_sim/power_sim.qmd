...
results <- data.frame(matrix(NA, nrow = n_sim, ncol = 1))
pvalues <- data.frame(matrix(NA, nrow = n_sim, ncol = 1))
...
for(i in 1:n_sim){
  ...
  if(one_sided == FALSE) {
    ...
    pvalues[i,1] <- coef(summary(model))[2,4]
    results[i,1] <- pvalues[i,1] < 0.05
  } else if (one_sided == TRUE) {
    # One sided test, halve the p-value
    pvalues[i,1] <- coef(summary(model))[2,4]/2

    if(side == 'right') {
      # Ensure test statistic is greater than the null hypothesis for right sided test
       results[i,1] <- pvalues[i,1] < 0.05 & coef(summary(model))[2,1] > 0
    }
    else if(side == 'left') {
      # Test stat less than null for left sided test
       results[i,1] <- pvalues[i,1] < 0.05 & coef(summary(model))[2,1] < 0
    } else {
      print('Error: one_sided must be TRUE or FALSE')
    }
  }
}
``` -->
...
# result = [power_sim(seed) for seed in random_grid]
...
# result = [pool.map_async(power_sim, seed) for seed in [1,2,3]]
...
The original use case of this experiment was to consider different treatment and control group sizes. The original power test analysis assumes equal sample sizes in treatment and control. Instead of looking for an appropriate statistical test, I can just reset the parameters above to have 70\% treatment and 30\% control proportions.
